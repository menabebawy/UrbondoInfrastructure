...
import static software.amazon.awscdk.services.apigatewayv2.alpha.HttpMethod.ANY;
...
public class UrbondoStack extends Stack {
    ...
    public UrbondoStack(@Nullable Construct scope, @Nullable String id, @Nullable StackProps props) {
        ...
        new CfnOutput(this, "HttApi", CfnOutputProps.builder().description("Url for Http Api").value(restApi.getUrl()).build());
    }

    private Function createLambdaFunction() {
        return new Function(this, "PrintRequest", FunctionProps.builder().runtime(Runtime.JAVA_17).code(Code.fromAsset("../lambda/build/distributions/lambda-0.1.zip")).handler("com.urbondo.PrintApiGatewayRequest").memorySize(1024).timeout(Duration.seconds(10)).logRetention(ONE_WEEK).build());
    }

    private RestApi createRestApi(Function lambdaFunction) {
        RestApiProps restApiProps = RestApiProps.builder().restApiName("Urbondo-REST-Api").deployOptions(StageOptions.builder().loggingLevel(INFO).build()).build();

        RestApi restApi = new RestApi(this, "Urbondo-REST", restApiProps);

        ResourceOptions resourceOptions = ResourceOptions.builder().defaultIntegration(LambdaIntegration.Builder.create(lambdaFunction).build()).build();
    ...
    private static void addResource(IResource rootResource, String path, ResourceOptions resourceOptions) {
        rootResource.addResource(path, resourceOptions).addMethod(POST.name()).getResource().addResource("{id}").addMethod(ANY.name());
    }
    ...
    private Table createTableBySuffix(String name) {
        ...
        TableProps tableProps = TableProps.builder().partitionKey(Attribute.builder().name(idAttributeName).type(AttributeType.STRING).build()).readCapacity(READ_CAPACITY).writeCapacity(READ_CAPACITY).removalPolicy(RemovalPolicy.DESTROY).tableName(prefix + "-" + name).build();
    ...
    private static GlobalSecondaryIndexProps globalSecondaryIndexProps(String attributeName) {
        return GlobalSecondaryIndexProps.builder().indexName(attributeName + "-index").partitionKey(Attribute.builder().name(attributeName).type(AttributeType.STRING).build()).readCapacity(READ_CAPACITY).writeCapacity(READ_CAPACITY).build();
    }
}